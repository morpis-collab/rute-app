...
/**
 * EmptyState - Komponen reusable untuk area kosong dengan tekstur ledger halus.
 * 
 * Fitur animasi:
 * 1. Floating Coffee Beans/Leaves - melayang organik via Framer Motion loop
 * 2. Wash halus - kedalaman ringan tanpa dekorasi berlebih
 ...
 * @param {boolean} showBlob - Tampilkan wash latar halus (default: false)
...
export default function EmptyState({
  ...
  const shouldShowParticles = showParticles && !shouldReduceMotion;
  const shouldShowBeans = showBeans && !shouldReduceMotion;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(79, 104, 79, ${p.opacity})`;
        ctx.fill();
      });
  ...
  return (
    <div className={`empty-state-container relative overflow-hidden rounded-xl bg-gradient-to-br from-white to-[var(--color-coffee-milk)] border border-dashed border-[var(--color-border)] ${sizeClasses[size]}`}>
      ...
      {showBlob && (
        <div className="empty-state-wash absolute inset-0 pointer-events-none z-0" aria-hidden="true">
          <div className="empty-wash empty-wash-1" />
          <div className="empty-wash empty-wash-2" />
          <div className="empty-wash empty-wash-3" />
        </div>
      )}
      ...
      <div className="relative flex flex-col items-center justify-center gap-2.5 px-4 text-center z-10">
        ...
        <motion.p
          ...
          className="empty-state-title max-w-xs text-sm font-semibold leading-snug text-[var(--color-text-secondary)]"
        >
          {message}
        </motion.p>
        ...
        {ctaLabel && onCtaClick && (
          <motion.button
            ...
            transition={softSpring}
            onClick={onCtaClick}
            className="mt-2 min-h-[44px] rounded-[var(--radius-button)] bg-[var(--color-band-1)] px-4.5 py-2 text-[11px] font-black text-white shadow-sm transition-all hover:shadow"
          >
            {ctaLabel}
          </motion.button>
        )}
