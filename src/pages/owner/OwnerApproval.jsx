            ...
            <div key={exp.id} className="kpi-card">
              ...
              <div className="border-t border-[var(--color-border)] pt-2 mt-2 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleAction(exp.id, 'reject')} className="btn btn-danger px-3 py-2 text-xs flex items-center gap-1"><X size={14} /> Tolak</button>
                  <button onClick={() => handleAction(exp.id, 'approve')} className="btn btn-success px-3 py-2 text-xs flex items-center gap-1"><Check size={14} /> Setujui</button>
                </div>
              </div>
            </div>
          ))}
